...
function assertAccess(i,u){
  ...
  if (u?.id === 'RESP-001' || role === 'RESPONDER') return; // Permanent emergency responder has access to handle incidents
  if (['DEPARTMENT_HEAD','TEACHER'].includes(role)){
...
export async function deleteIncident(id, u, ip = '') {
  ...
  if (u?.id !== 'RESP-001' && !['RESPONDER', 'TEACHER', 'INSTITUTE_ADMIN', 'SUPER_ADMIN', 'ADMIN'].includes(uRole)) {
